...
const TalentezzyUsernameInput = (props: Props) => {
  const { isSubmitted, isLoading, inputValue, emitInputValue } = props;
  const [value, setValue] = useState('');
  const [isValid, setValid] = useState(false);
  const [errorMessage, setErrorMessage] = useState('Enter a valid username.');
  ...
  const handleFormOnChange = (event: any) => {
    ...
    setValue(val);
    setErrorMessage(!valid ? 'Enter a valid username.' : '');
...
export default TalentezzyUsernameInput;
